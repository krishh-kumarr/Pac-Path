...
const PacmanGame = () => {
  ...
  const [score, setScore] = useState(0); // Score now tracks blocks travelled
  const [gameStarted, setGameStarted] = useState(false);
  const [isMoving, setIsMoving] = useState(false);
  const [placingFood, setPlacingFood] = useState(false);
  const [animatedCells, setAnimatedCells] = useState([]); // Track animated cells
  ...
  const initializeGame = () => {
    ...
    // Add random walls (avoiding start position)
    for (let i = 0; i < 50; i++) {
    ...
    setPacman({ x: 1, y: 1 });
    setFood(null);
    setScore(0); // Reset score on game initialization
    setPath([]);
    setIsMoving(false);
    setGameStarted(false);
    setPlacingFood(true);
    setAnimatedCells([]); // Initialize animatedCells
  };
  ...
  const handleCellClick = (x, y) => {
    if (placingFood && !walls.includes(`${x},${y}`) && (x !== pacman.x || y !== pacman.y)) {
      setFood({ x, y });
  ...
  useEffect(() => {
    ...
    const movePacman = async () => {
      ...
      if (newPath && newPath.length > 1) {
        setPath(newPath);
        setIsMoving(true);
        setAnimatedCells([]); // Clear previous animations

        for (let i = 1; i < newPath.length; i++) {
          const pos = newPath[i];
          setPacman(pos);
          setScore(prev => prev + 1); // Increment score with each block travelled
          setAnimatedCells(prev => [...prev, { x: pos.x, y: pos.y }]); // Add to animated cells
          await new Promise(resolve => setTimeout(resolve, 300));
        }

        if (newPath[newPath.length - 1].x === food.x &&
          newPath[newPath.length - 1].y === food.y) {
          //setScore(prev => prev + 100); // No longer needed
          setPlacingFood(true);
          setFood(null);
        }
        setIsMoving(false);
        setAnimatedCells([]); // Clear animations after done moving
      }
    };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black p-4">
      <div className="mb-4 text-white text-2xl">Blocks Travelled: {score}</div> {/* Updated label */}
